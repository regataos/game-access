...
function openUrl() {
	const popup = window.open("./../pages/go-gog.html", 'popup')
...
function saveLoginId(loginId) {
	...
	setTimeout(function () {
		if (!fs.existsSync("/opt/wine-gcs/bin/wine")) {
			...
			/opt/regataos-gcs/scripts/show-gog-games.sh & \
			/opt/regataos-gcs/scripts/install/scripts-install/install-game-gog/prepare-compatibility-mode.sh';
		} else {
			commandLine = '/opt/regataos-gcs/scripts/show-gog-games.sh & \
			/opt/regataos-gcs/scripts/install/scripts-install/install-game-gog/prepare-compatibility-mode.sh';
		}
...
// Verify that GOG login has already been performed
...
function hideInstallButtonLauncher() {
	...
	const seeMoreButtonGog = document.querySelector(".gog-more");
	const installButtonGog = document.querySelector(".gog-install");
	const installButtonGog2 = document.querySelector(".install-button-gog");

	if (installedLaunchers.includes("gog")) {
		seeMoreButtonGog.classList.add("show-element");
		seeMoreButtonGog.classList.remove("hide-element");

		installButtonGog.classList.remove("show-element");
		installButtonGog.classList.add("hide-element");

		installButtonGog2.classList.add("button-effect-disabled");
	} else {
		seeMoreButtonGog.classList.remove("show-element");
		seeMoreButtonGog.classList.add("hide-element");

		installButtonGog.classList.add("show-element");
		installButtonGog.classList.remove("hide-element");

		installButtonGog2.classList.remove("button-effect-disabled");
	}
}
hideInstallButtonLauncher();

// Check if the user is logged in and if the games available in the GOG 
...
function hideLoginScreen() {
	...
	function showGameBlocks() {
		const showInstalledGamesFile = "/tmp/regataos-gcs/config/installed/show-installed-games-gog.txt";

		handleCssClass("add", "show-games", "list-account-games");
		handleCssClass("add", "hide-element", ["loading", "loading-games", "gog-login"]);
		handleCssClass("remove", "grid-element", "gog-login");
		...
		handleCssClass("remove", "body-gog-background", "body-page");
	...
	function showLoginScreen() {
		if (fs.existsSync('/tmp/regataos-gcs/gog-login-code.txt')) {
			handleCssClass("remove", "grid-element", "gog-login");
			handleCssClass("remove", "body-gog-img", "body-page");
			handleCssClass("add", "show-element", ["loading", "loading-games"]);
		} else {
			handleCssClass("remove", "hide-element", "gog-login");
			handleCssClass("add", "grid-element", "gog-login");
			handleCssClass("add", "body-gog-img", "body-page");
			handleCssClass("remove", "show-element", ["loading", "loading-games"]);
		}
	...
	const oldcheckUserLoginFile = "/tmp/regataos-gcs/config/gog-games/show-games.txt";
...
function detectLoggedAccount() {
	const fs = require("fs");
	if (!fs.existsSync("/tmp/regataos-gcs/config/gog-games/show-games.txt")) {
