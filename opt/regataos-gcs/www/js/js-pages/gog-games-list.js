...
if (!err) {
	...
	gog_games_json.forEach(gamesdata => {
		const child = gog_all_games.querySelector("div." + gamesdata.gamenickname + "-block");
		...
		if (child == null) {
			...
			<div class="universal-game-img gog-game-img" style="background-image: url(' + gamebackg + ')"></div> \
				<div class="block-play-universal"> \
					<div id="' + gamesdata.gamenickname + '" class="install-box-universal" onclick="window.gamenickname=this.id; install_gog_game();"> \
