...
app_name_down="Baixando $GAME_NAME"
app_name_process="Instalar $GAME_NAME"
app_install_status="Instalando $GAME_NAME..."
...
function install_app() {
	...
	/opt/regataos-gcs/legendary/legendary import-game $GAME_ID "$GAME_INSTALL_DIR/"
}
...
function success_installation() {
	...
	# Notify
	notify-send -i emblem-ok-symbolic -u normal -a 'Regata OS Game Access' "$GAME_NAME $success_notify_title" "$GAME_NAME $success_notify_text"
}
...
function installation_failed() {
	...
	# Notify
	notify-send -i emblem-important-symbolic -u normal -a 'Regata OS Game Access' "$error_notify_title $GAME_NAME!" "$error_notify_text $GAME_NAME."
}
