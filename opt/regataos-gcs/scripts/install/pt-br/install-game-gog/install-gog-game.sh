...
#General information
launcher="gog"
launcher_name="GOG Games"

if [ -z $game_nickname ]; then
	if test -e "/tmp/regataos-gcs/start-installation-$launcher.txt"; then
		export game_nickname="$(cat /tmp/regataos-gcs/start-installation-$launcher.txt)"
		rm -f "/tmp/regataos-gcs/start-installation-$launcher.txt"
...
app_name="$(grep -r "gamename" $HOME/.config/regataos-gcs/$launcher-games/json/$game_nickname-$launcher.json | cut -d":" -f 2- | sed 's/ //' | sed 's/"\|,//g')"
game_id="$(grep -r "gameid" $HOME/.config/regataos-gcs/$launcher-games/json/$game_nickname-$launcher.json | cut -d":" -f 2- | sed 's/ //' | sed 's/"\|,//g')"
game_folder="$(grep -r "game_folder" $HOME/.config/regataos-gcs/$launcher-games/json/$game_nickname-$launcher.json | cut -d":" -f 2- | sed 's/ //' | sed 's/"\|,//g')"
app_nickname="$launcher"
...
function create_installation_directory() {
	...
	if [ -z "$installation_folder" ]; then
		mkdir -p "$HOME/Game Access/$launcher_name"
		GAME_INSTALL_DIR="$HOME/Game Access/$launcher_name"

	else
		if [[ $(echo $installation_folder) == *"game-access/$launcher_name"* ]]; then
			install_folder="$(echo $installation_folder | sed 's|/game-access/$launcher_name||')"
		...
		if test ! -e "$install_folder/game-access/$launcher_name"; then
			mkdir -p "$install_folder/game-access/$launcher_name"
		fi

		GAME_INSTALL_DIR=$(echo "$install_folder/game-access/$launcher_name")

		if test ! -e "$HOME/Game Access/$launcher_name"; then
...
function success_installation() {
	cp -f "$HOME/.config/regataos-gcs/$launcher-games/json/$game_nickname-$launcher.json" \
	"$HOME/.config/regataos-gcs/installed/$game_nickname-$launcher.json"
...
if test -e "$progressbar_dir/installing"; then
	if test ! -e "/tmp/regataos-gcs/installing-$game_nickname"; then
		...
		if [[ $kmsg == *"$game_nickname"* ]]; then
			...
			if [[ $(echo $GAME_INSTALL_DIR) != *"$HOME/Game Access/$launcher_name"* ]]; then
				echo "$GAME_INSTALL_DIR" >"/tmp/regataos-gcs/$game_nickname-installdir.txt"
			fi

			echo "$game_nickname=$launcher process-$app_name_process" >>$progressbar_dir/queued-process
		fi
...
killall install-$launcher-game.sh
pkill --signal CONT gogdl
killall gogdl
...
EOM
	...
	echo "gogdl" >$progressbar_dir/tool-pid

	# Prepare installation directory 
	create_installation_directory

	/opt/regataos-gcs/tools/gogdl/gogdl download "$game_id" --platform windows --base-path "$GAME_INSTALL_DIR/" &> "$progressbar_dir/download-percentage-gogdl"
...
if [[ $(ps aux | egrep "install-$launcher-game.sh") == *"install-$launcher-game.sh start"* ]]; then
