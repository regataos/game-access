...
app_name_down="Downloading Rockstar Games Launcher"
app_name_process="Install Rockstar Games Launcher"
app_install_status="Installing Rockstar Games Launcher..."
...
function install_app() {
	...
	wine-gcs /tmp/regataos-gcs/$app_download_file_name
}
...
function success_installation() {
	...
	rm -rf "$HOME/.local/share/applications/Programs/Rockstar Games"
...
function gameinstall_folder() {
	if test -e "$HOME/.config/regataos-gcs/external-games-folder.txt"; then
		...
		if test -e "$(echo $external_directory)/wineprefixes-gcs"; then
			...
			cp -rf "$app_nickname_dir/drive_c/Program Files" "$(echo $external_directory)/$app_name/"
			rm -rf "$app_nickname_dir/drive_c/Program Files"
			ln -sf "$(echo $external_directory)/$app_name/Program Files" "$app_nickname_dir/drive_c/"
		else
